...
const isCorrectionInProgress: Ref<boolean> = ref(false);

const correctMessage = () => {
  if (
    isCorrectionInProgress.value ||
    !isUserMessage.value ||
  ...
  isCorrectionInProgress.value = true;
  ...
  messageCorrector
    ...
    .then((correctedMessage: Message) => {
      if (props.message.uid === correctedMessage.uid) {
        return;
      }
    ...
    .finally(() => {
      isCorrectionInProgress.value = false;
    });
};
</script>
...
<template>
  ...
  >
    ...
    <div
      class="flex min-w-[20rem] p-2 rounded-lg mb-2 relative text-gray-800 border"
      :class="
        isUserMessage
    ...
    >
        ...
        <div>
          ...
          <div class="float-right">
            <a
              v-if="isUserMessage"
              href="#"
              class="text-sm inline-block float-right mx-2"
              :class="
                props.message.correctedContent.length > 0
                  ? 'text-red-500'
                  : 'text-green-500'
              ...
              @click="correctMessage"
            >
              <fa
                v-if="!isCorrectionInProgress"
                :icon="['fas', 'pencil-alt']"
              />
              ...
              <template #content>
                <div class="flex flex-col text-sm">
                  ...
                  <div class="flex flex-row flex-wrap">
                    ...
                    <div class="w-5/6 border px-4 py-2">
                      <PreJson :content="message.correctedMessageList || ''" />
                    </div>
                  </div>
                </div>
              </template>
      ...
      <div
        v-if="!isUserMessage"
        class="absolute left-0 top-1/2 transform -translate-x-1/2 rotate-45 w-4 h-4 bg-violet-200"
      ></div>
      <div
        v-else
        class="absolute right-0 top-1/2 transform translate-x-1/2 rotate-45 w-4 h-4 bg-pink-200"
      ></div>
    </div>
  </div>
</template>
